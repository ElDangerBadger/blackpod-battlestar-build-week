# Historical Build Week implementation reference

> Superseded on 2026-09-15 by [PRODUCT_SPEC.md](PRODUCT_SPEC.md) and
> [LIVE_PRODUCT_RUNBOOK.md](LIVE_PRODUCT_RUNBOOK.md). The material below records
> the original submission and its command contracts; it is not current product
> scope or startup guidance. In particular, demo-default, approved-only
> packaging, and “no web service” statements describe the historical release.
> Legacy mission commands can write mission state; `make live-mission` includes
> `APPROVE_HANDOFF`. They are not part of read-only Cabin startup.
...
- [Captain's Cabin](CAPTAINS_CABIN.md)
- [Navigator V3 source mapping and update checks](NAVIGATOR_V3_INTEGRATION.md)
- [Stage 4 LIVE Demo Runbook](LIVE_DEMO_RUNBOOK.md)
...
are described in [Captain's Cabin](CAPTAINS_CABIN.md).
...
[Stage 4 LIVE Demo Runbook](LIVE_DEMO_RUNBOOK.md) for the exact variables,
...
[Captain's Cabin](CAPTAINS_CABIN.md) for its architecture and known data
gaps.
