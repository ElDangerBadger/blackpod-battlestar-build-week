...
describe("chartStretchX", () => {
  it("keeps the cinematic ocean at true scale through the chart threshold", () => {
    expect(chartStretchX(0)).toBe(1);
    expect(chartStretchX(0.4)).toBe(1);
    expect(chartStretchX(0.55)).toBe(1);
  });

  it("increases monotonically and reaches the canonical V3 maximum", () => {
    let previous = chartStretchX(0.55);
    for (let value = 0.6; value <= 1.0001; value += 0.05) {
      const current = chartStretchX(value);
      expect(current).toBeGreaterThanOrEqual(previous);
      previous = current;
    }
    expect(chartStretchX(1)).toBe(14);
  });
});
...
describe("projectNavigatorOcean", () => {
  ...
  it("never exceeds maxPoints across uneven history lengths", () => {
    for (const count of [2, 3, 7, 499, 501, 733, 999, 1_001, 5_003]) {
      const projection = projectNavigatorOcean(marketWithPoints(count), { maxPoints: 127 });
      expect(projection?.sampled.length).toBeLessThanOrEqual(127);
      expect(projection?.sampled.at(-1)?.t).toBe(count);
    }
  });
