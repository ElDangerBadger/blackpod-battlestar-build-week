...
/**
 * Lateral price-axis stretch used by Navigator V3's transition into its flat
 * chart. Values remain unmodified; this is only a world-space presentation
 * transform.
 */
export function chartStretchX(viewT: number): number {
  const progress = Math.max(0, Math.min(1, (viewT - 0.55) / 0.45));
