...
export function NavigatorOceanBoundary({
  ...
  const fallbackReason = props.data.points.length < 2
  ...
  const fallback = <OceanFallback props={props} reason={fallbackReason} />;

  if (!capability || runtimeUnavailable || props.data.points.length < 2) return fallback;
