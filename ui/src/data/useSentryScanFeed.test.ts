...
import { SENTRY_SCAN_TIMEOUT_MS, useSentryScanFeed } from "./useSentryScanFeed";

vi.mock("./sentryScanFeed", () => ({ loadSentryScanFeed: vi.fn() }));
const loader = vi.mocked(loadSentryScanFeed);
const flush = async () => { await act(async () => { await Promise.resolve(); }); };
const unavailable = (status: "NOT_CONFIGURED" | "UNAVAILABLE"): SentryScanFeed => ({
  ...createSentryScanFixture(), status, source: null, scan: null,
  message: "Scan archive unavailable.",
});

describe("Sentry Scan reader lifecycle", () => {
  beforeEach(() => {
    vi.useFakeTimers(); vi.setSystemTime(new Date("2026-09-18T08:00:00Z"));
    loader.mockReset().mockResolvedValue(createSentryScanFixture());
    vi.spyOn(document, "visibilityState", "get").mockReturnValue("visible");
  });
  afterEach(() => { vi.useRealTimers(); vi.restoreAllMocks(); });

  it("reads once on opening and once per manual refresh, without polling", async () => {
    const { result, unmount } = renderHook(() => useSentryScanFeed({ enabled: true }));
    expect(result.current.status).toBe("LOADING");
    await flush();
    expect(result.current.status).toBe("READY");
    expect(result.current.feed?.scan?.scan_kind).toBe("OFFLINE_RESEARCH_SNAPSHOT");
  ...
  it("never reads while disabled or replayed, including manual refresh", async () => {
    const { result } = renderHook(() => useSentryScanFeed({ enabled: false }));
    act(() => result.current.refresh()); await flush();
  ...
  it.each(["NOT_CONFIGURED", "UNAVAILABLE"] as const)("does not invent results for %s", async (status) => {
    ...
    const { result } = renderHook(() => useSentryScanFeed({ enabled: true }));
    await flush();
  ...
  it.each(["NOT_CONFIGURED", "UNAVAILABLE"] as const)("retains old evidence with a distinct %s server status", async (status) => {
    ...
    expect(result.current.status).toBe(status);
    expect(result.current.feed).toBe(prior);
    expect(result.current.message).toBe("Scan archive unavailable.");
  });

  it("does not roll evidence back to an out-of-order reader receipt", async () => {
    const { result } = renderHook(() => useSentryScanFeed({ enabled: true }));
    await flush(); const prior = result.current.feed;
    loader.mockResolvedValue({ ...createSentryScanFixture(), checked_at: "2026-09-18T04:59:59Z" });
    act(() => result.current.refresh()); await flush();
    expect(result.current.status).toBe("UNAVAILABLE");
    expect(result.current.feed).toBe(prior);
  });
  ...
  it("bounds a hung read, aborts it, and ignores a late result without automatic retry", async () => {
    let resolveFirst!: (value: SentryScanFeed) => void;
    loader.mockImplementationOnce(() => new Promise((resolve) => { resolveFirst = resolve; }));
    const { result } = renderHook(() => useSentryScanFeed({ enabled: true }));
    await act(() => vi.advanceTimersByTimeAsync(SENTRY_SCAN_TIMEOUT_MS - 1));
    expect(result.current.status).toBe("LOADING");
    await act(() => vi.advanceTimersByTimeAsync(1));
    expect(result.current.status).toBe("UNAVAILABLE");
    expect(loader.mock.calls[0][0]?.signal?.aborted).toBe(true);
    resolveFirst(createSentryScanFixture()); await flush();
    expect(result.current.feed).toBeNull();
    await act(() => vi.advanceTimersByTimeAsync(3_600_000));
    expect(loader).toHaveBeenCalledTimes(1);
  });
  ...
  it("aborts on close and keeps a late result from crossing a reopen", async () => {
    let resolveFirst!: (value: SentryScanFeed) => void;
    loader.mockImplementationOnce(() => new Promise((resolve) => { resolveFirst = resolve; }));
    const { result, rerender } = renderHook((props) => useSentryScanFeed(props), { initialProps: { enabled: true } });
    const signal = loader.mock.calls[0][0]?.signal;
    rerender({ enabled: false });
    expect(signal?.aborted).toBe(true);
    rerender({ enabled: true }); await flush();
    expect(result.current.status).toBe("READY");
    resolveFirst({ ...createSentryScanFixture(), checked_at: "2026-09-18T07:30:00Z" }); await flush();
    expect(result.current.feed?.checked_at).toBe("2026-09-18T07:00:00Z");
  });
